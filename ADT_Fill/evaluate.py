...
def test_all_users(model, batch_size, item_num, test_data_pos, user_pos, top_k):
    ...
    precision, recall, F1, NDCG, MRR = compute_acc(GroundTruth, predictedIndices, top_k)
    ...
    for index in range(len(topN)):
        ...
        for i in range(len(predictedIndices)):  # for a user,
            if len(GroundTruth[i]) != 0:
                ...
                user_precision = userHit / topN[index]
                sumForPrecision += user_precision
                user_recall = userHit / len(GroundTruth[i])
                sumForRecall += user_recall
                if (user_precision+user_recall)!=0:
                    sumForF1+=2*((user_precision*user_recall)/(user_precision+user_recall))
                else:
                    sumForF1+=0
                sumForNdcg += ndcg
                sumForMRR += userMRR
        ...
        F1.append(sumForF1/len(predictedIndices))
